...
public final class PatchingResourceProvider implements ResourceFactory {
	...
	public Optional<Resource> getResource(Identifier identifier) {
		...
		if (resource.isEmpty()) {
			...
			try (final InputStream inputStream = resource.get().getInputStream()) {
				...
				return Optional.of(new Resource(resource.get().getResourcePackName(), () -> new ByteArrayInputStream(returningContent.getBytes(StandardCharsets.UTF_8))));
			} catch (Exception e) {
